...
The chain is a series of layers, each with one job, passing a well-defined data shape to the next. The layers we control fully are the text recognition, the geometry resolution, and the renderer — **build these first.**
...
Student photo
      │
      ▼
[1] Read the page — Mathpix (shapes) + GPT-5 (text), fused
      │
      ▼
[2] IR builder — package what was read into a structured form
      │
      ▼
[3] Evaluator (ON HOLD — org's model) — judge correctness, using the question for context
      │
      ▼
[4] Span → Geometry — turn feedback positions into shapes on the image
      │
      ▼
[5] Annotorious renderer — draw the interactive annotations
      │
      ▼
Interactive annotated image (the uploaded photo is what the student sees)
```

**Build order note.** Layers 1, 2, and 5 are built now. Layer 4's *core* (turning a known position into shapes) is built now against fixture data. Layer 3 (the evaluator) and the part of Layer 4 that interprets the evaluator's real output are **on hold** — see those layer descriptions. The pipeline runs end-to-end during development by putting fixture feedback where the evaluator will go, so no layer is blocked waiting for it.

### Layer descriptions

**[1] Read the page — Mathpix (shapes) + GPT-5 (text), fused.**
Two readers look at the uploaded photo, each trusted for the one thing it does best. Mathpix is trusted for *where* the writing is: it returns four-corner shapes that follow each line and word, and reads maths as structured notation (LaTeX). It reads the page two ways — a line reading (how many lines, in reading order) and a word reading (the exact shape of every word) — combined so each line carries its precise word shapes. GPT-5 is trusted for *what* the writing says: the actual text, read far more accurately on messy handwriting than Mathpix manages. Fusion then lays GPT-5's text onto Mathpix's shapes, line by line, so every shape carries the right text in the right place. This layer reads everything on the page — including any question the student may have copied onto their answer sheet. Deciding what to ignore is not this layer's job; it reads, and passes everything forward.

**[2] IR builder — package what was read.**
This layer takes the fused reading and packages it into our own consistent structure (the "Intermediate Representation"). It records, for each line of writing: the text, the maths notation, the shape on the image, and its position as a character range within one continuous text string. It also produces that continuous text string with line breaks preserved. Everything downstream reads from this structure, not from the readers directly — so if we ever change reader, only this layer changes.

**[3] Evaluator — judge correctness. ⏸ ON HOLD.**
...
**[4] Span → Geometry — turn feedback positions into shapes.**
...
*On hold (with Layer 3):* the front part of this layer that takes the **real** evaluator's output and turns it into a position. If the evaluator points by span-text, this part must handle the maths formatting mismatch and the case where the same text appears more than once; if the evaluator points by exact position, this part is trivial or unnecessary. Because these are opposite amounts of work, we do not build it until we know which evaluator we have.

**[5] Annotorious renderer — draw the interactive annotations.**
The frontend takes the uploaded photo and the resolved regions and draws them as an overlay. Each region is a shape following the writing, tinted by category. Selecting a region shows its feedback in a panel. Regions are shapes that follow the writing (not plain rectangles), so they sit correctly on the page. Feedback with no region is listed in the panel on its own.
